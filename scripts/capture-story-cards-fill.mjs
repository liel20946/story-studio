#!/usr/bin/env node
/**
 * Capture Story view cards filling to the bottom (matching Run view).
 * Prerequisites: npm run build && npm run seed:demo
 *   STORY_STUDIO_MOCK_RUNS=1 xvfb-run -a node scripts/capture-story-cards-fill.mjs
 */
...
async function main() {
  ...
  // Open Login Flow (seeded story with steps + variables).
  const storyRow = page.getByText("Login Flow", { exact: true }).first();
  await storyRow.click({ force: true });
  await wait(1000);
  await shot(app, "01-story-view-cards-fill");

  // Open a finished run for side-by-side consistency check.
  await page.getByRole("tab", { name: "Runs" }).click({ force: true });
  await wait(600);
  const runRow = page.locator(".sidebar-scroll").getByText(/Login Flow|Checkout|Onboarding|Settings/i).first();
  await runRow.click({ force: true });
  await wait(1000);
  await shot(app, "02-run-view-cards-fill");
