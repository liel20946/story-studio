#!/usr/bin/env node
/**
 * Capture screenshots for sidebar system-actions restructure:
 *  Stories|Bulk tabs, History/Scheduled/Search actions, Plus menu
 *
 * Prerequisites: npm run build && npm run seed:demo
 *   STORY_STUDIO_MOCK_RUNS=1 xvfb-run -a node scripts/capture-sidebar-system-actions.mjs
 */
...
async function captureSuite(theme) {
  ...
  const prefix = `sidebar-restructure-${theme}`;
  ...
  // 5. Open a history run if present
  const historyRow = page.locator(".home-link-row").first();
  if (await historyRow.count()) {
    await historyRow.click({ force: true });
    await wait(1000);
