#!/usr/bin/env node
/**
 * Capture Settings → Agent browser options for Codex vs Claude Code providers.
 * Prerequisites: npm run build && npm run seed:demo
 *   STORY_STUDIO_MOCK_RUNS=1 xvfb-run -a node scripts/capture-browser-options-by-provider.mjs
 */
...
async function openAgentSettings(page) {
  ...
  await page.getByText("Browser", { exact: true }).first().waitFor();
}
...
async function main() {
  ...
  // Codex provider — all three browser options
  const provider = page.locator('[aria-label="Agent provider"]');
  const codexTab = provider.getByRole("tab", { name: "Codex" });
  if (!(await codexTab.getAttribute("aria-selected").then((v) => v === "true"))) {
    await codexTab.click({ force: true });
    await wait(400);
  }
  await wait(400);
  await shot(app, "01-browser-options-codex-provider");

  // Claude Code provider — only Private + Playwright
  await provider.getByRole("tab", { name: "Claude Code" }).click({ force: true });
  await wait(600);
  await shot(app, "02-browser-options-claude-provider");
