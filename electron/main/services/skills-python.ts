...
export async function convertPlaywrightRecording(
  specPath: string,
  outputDir: string,
  url: string,
  name?: string,
): Promise<{ draftMd: string; draftYaml: string }> {
  await fs.mkdir(outputDir, { recursive: true });
  const mdOut = path.join(outputDir, "draft.story.md");
  const yamlOut = path.join(outputDir, "draft.story.yaml");
  const args = [
    specPath,
    "--url",
    url,
  ...
  await runPythonScript("convert_playwright_recording.py", args);
  const draftMd = await fs.readFile(mdOut, "utf-8");
  const draftYaml = await fs.readFile(yamlOut, "utf-8");
  return { draftMd, draftYaml };
}
